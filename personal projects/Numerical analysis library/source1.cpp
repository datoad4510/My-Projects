...
int main()  //maybe add gauss_inverse and det_inverse (using adjugate) and compare times?
{
	...
	Matrix<double> A(2, 2);//determninant works, gauss works, minor/adjugate work, rank/num_sol prolly work but need to be tested more(not completely sure about num_sol), inverse works  now (equality numerical errors fixed), adj_inverse works but is O(n^5)
	A[0][0] = 1;
	A[0][1] = 1;
	A[1][0] = 1;
	A[1][1] = -1;
	...
	b[0][0] = 1;
	b[1][0] = 20;
	Matrix<double> C = A.gauss_solve(b);
	C.print(cout);
	system("pause");
}
