...
#include<iostream>
#include<queue>
using namespace std;//kitxva: sxvanairi insert ratom ar mushaobs??!
...
private:
	...
	link select(link, int) noexcept;
	int rank(link) noexcept;
public:
	...
	int tree_size() noexcept { return root->size; }
...
link BinarySearchTree::select(link x, int i) noexcept
{
	int r = (x->left)->size + 1;
	if (i == r) return x;
...
int BinarySearchTree::rank(link x) noexcept
{
	int r = (x->left)->size + 1;
	link y = x;
	while (y != root) {
...
void BinarySearchTree::insert(int d) noexcept
{
	...
	link t = new tree_node(d);//new rom ar gagveketebina, lokaluri cvladi iqneboda. magalitad. tree_node tr(d); link t = &tr; ar imushavebda, radgan waishleboda funqciis damtavrebis mere.
	link y = nullptr;
	link x = root;
	...
	while (nullptr != x)
	{
		y = x;
		if (t->data < x->data) {
			x = x->left; //sheidzleboda aq x->left da qvemot x-> right. es duplicatebs sxvanairad daalagebda, yvelafer danarchens igivenairad. exla duplicatebi marjvniv midis. magram, duplikatebi roca marcxniv midis ar mushaobs sworad!!! yvela araa
			++(y->size); // x->p doesnt make sense if x is a nullptr. because of this, we need to have a copy of its parent!
		}
		//updating sizes
		else {
			x = x->right; //if(x != nullptr) { cout << endl << x->data << "->size == " << x->size << endl; }
			++(y->size);
		}
	}
	t->p = y;
	...
	{
		root = t;
		++(root->size);//pirveli obieqtis chasmis drosac sachiroa 1-it gazrda
		return;
	}
	if (d < y->data)
		y->left = t;
...
{
	inorder(root, [](tree_node* p) {std::cout << " " << p->data << ":" << p->size << " "; });
	cout << endl;
}

void BinarySearchTree::print_preorder() noexcept
{
	preorder(root, [](tree_node* p) {std::cout << " " << p->data << ":" << p->size << " "; });
	cout << endl;
}

void BinarySearchTree::print_postorder() noexcept
{
	postorder(root, [](tree_node* p) {std::cout << " " << p->data << ":" << p->size << " "; });
	cout << endl;
}

void BinarySearchTree::print_outorder() noexcept
{
	outorder(root, [](tree_node* p) {std::cout << " " << p->data << ":" << p->size << " "; });
	cout << endl;
}

void BinarySearchTree::print_levelorder() noexcept
{
	levelorder(root, [](tree_node* p) {std::cout << " " << p->data << ":" << p->size << " "; });
	cout << endl;
}

BinarySearchTree::~BinarySearchTree()
{
	inorder(root, [](link p) { delete p; }); //insert naxe da mixvdebi ratoa delete
	root = nullptr;
}
